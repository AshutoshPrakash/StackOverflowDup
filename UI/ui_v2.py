...
def removePlaceholder(*event):
    if placeholder.get()=="Filter Tags":
        placeholder.set("")

def filterTags(*event):
   print placeholder.get()+event[0].char
   if event[0].char=="":
       #backspace is pressed
       listbox.delete(0, END)
       for i in tags:
          if placeholder.get()[:-1] in i:
             listbox.insert(END, i)
   else:
       listbox.delete(0, END)
       for i in tags:
          if placeholder.get()+event[0].char in i:
             listbox.insert(END, i)
...
def fetch(entries):
   ...
   for entry in entries:
      ...
      if text is "":
          ...
          v+= 1
   if v==2:
       if tag1_val.get() == "None" and tag2_val.get() == "None" and tag3_val.get() == "None" and tag4_val.get() == "None" and tag5_val.get() == "None":
          tkMessageBox.showinfo("Error","Set Atleast One Tag!")
          status_val.set("Atleast one Tag is need to be set")
       elif check()==1:
...
def makeform(root, fields):
   ...
   placeholder.set('Filter Tags')
   suggest = Entry(bottomframe2, textvariable =placeholder)
   suggest.bind('<Button-1>', removePlaceholder)
   suggest.bind('<Key>',filterTags)
   suggest.bind('<BackSpace>',filterTags)
   suggest.pack(side=BOTTOM)
...
def removeChoice(*event):
    try :
        ...
        status_val.set("Nothing selected !")
