...
aa = 10
k = 0
for j in range(1,int(round(n_groups+1)/aa)):
	sum1 = sum2 = sum3 = 0
	for i in range((j-1)*10+1,(j)*10):
		sum1 += arr_original[i]
		sum2 += arr_vote1[i]
		sum3 += arr_vote4[i]
		
	plot_original.insert(j,sum1)
	plot_vote1.insert(j,sum2)
	plot_vote4.insert(j,sum3)
	keys.insert(j,str(j*10))
	k += 1
...
if str(sys.argv[1])=='original':
	rects1 = plt.bar(index, tuple(plot_original), bar_width,alpha=opacity,color='b',label='Original Questions')
	...
	rects2 = plt.bar(index + bar_width, tuple(plot_vote1), bar_width,alpha=opacity,color='r',label='Duplicate Question with vote=1')
	plt.title('Questions Description Word Count')
	plt.xlabel('Word Count Range')
	plt.ylabel('Fraction of Questions')
	...
	rects3 = plt.bar(index + bar_width+ bar_width, tuple(plot_vote4), bar_width,alpha=opacity,color='c',label='Duplicate Question with vote>4')
