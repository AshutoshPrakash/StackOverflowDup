...
with open("originalQuestions.txt") as f2:
	for line in f2:
		...
		p = cursor.fetchall()
		if len(p) !=0:
			if len(p[0][0].split(' ')) in original_count:
				original_count[len(p[0][0].split(' '))] += 1
				o+= 1
			else:
				original_count[len(p[0][0].split(' '))] = 1
...
with open("dupWithVote1.txt") as f:
    for line in f:
		...
		p = cursor.fetchall()
		if len(p) !=0:
			if len(p[0][0].split(' ')) in vote1_count:
				vote1_count[len(p[0][0].split(' '))] += 1
				v1+=1
			else:
				vote1_count[len(p[0][0].split(' '))] = 1
